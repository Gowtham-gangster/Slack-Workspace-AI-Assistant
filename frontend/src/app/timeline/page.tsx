...
import { Clock, Hash, Zap, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function TimelinePage() {
  ...
  const { data: events, isLoading, error, refetch } = useQuery<any[]>({
    queryKey: ['timelineEvents', selectedChannel, limit],
    queryFn: () => apiFetch(`/api/timeline?channelId=${selectedChannel}&limit=${limit}`),
  ...
  return (
    <AppLayout>
      <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 py-6 md:py-8">
        ...
        <div className="mb-6 md:mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">AI-generated timeline from Slack conversations</p>
            </div>
          </div>
        </div>

        {/* Controls */}
        <div className="flex gap-3 mb-8 p-4 rounded-2xl border bg-white/[0.03] border-white/[0.07]">
          <div className="relative flex-1">
            <Hash className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: '#9ca3af' }} />
            <select
              value={selectedChannel}
              onChange={e => setSelectedChannel(e.target.value)}
              className="w-full pl-9 pr-4 py-2.5 rounded-xl text-sm border outline-none appearance-none bg-white/[0.05] border-white/[0.1] text-slate-200"
            >
          ...
          <select
            ...
            className="px-3 py-2.5 rounded-xl text-sm border outline-none bg-white/[0.05] border-white/[0.1] text-slate-200"
          >
        ...
        {/* Timeline */}
        {!selectedChannel && (
          <div className="text-center py-20 rounded-2xl border border-white/[0.07] bg-white/[0.02]">
            <Clock className="w-12 h-12 mx-auto mb-4" style={{ color: '#374151' }} />
            <p className="text-base font-medium mb-1 text-slate-300">Select a Channel</p>
          ...
          <div className="space-y-4">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="flex gap-4 animate-pulse">
                <div className="w-12 h-4 rounded mt-1" style={{ background: 'rgba(255,255,255,0.08)' }} />
                <div className="flex flex-col items-center">
                  <div className="w-3 h-3 rounded-full" style={{ background: 'rgba(255,255,255,0.12)' }} />
                  <div className="w-0.5 h-16 mt-1" style={{ background: 'rgba(255,255,255,0.05)' }} />
                </div>
                    ...
                    <div className="relative z-10 shrink-0 mt-3">
                      <div className="w-3 h-3 rounded-full ring-4 transition-all group-hover:scale-125"
                           style={{
                             ...
                             boxShadow: '0 0 0 4px #06070d',
                           }} />
                    </div>

                    {/* Card */}
                    <div
                      className="flex-1 p-4 rounded-2xl border cursor-pointer transition-all duration-200 bg-white/[0.03] border-white/[0.07] hover:bg-white/[0.05]"
                        ...
                        <div className="flex items-start gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                              ...
                              <span className="text-[10px] font-medium animate-fadeIn" style={{ color: '#9ca3af' }}>
